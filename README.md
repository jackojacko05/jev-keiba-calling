...
この知覚段階と候補生成では実況上の重要度・優先順位・新規性を付けません。全頭位置、現在カメラ内の個体、前フレームとの差分、画面全体の事実を機械的に同じ候補群へ変換し、Jevなしは固定LLM、JevありはJevがそれぞれ1件を選びます。その後は同じ固定LLMが選択済み事実だけを実況文にします。実況文は固定高さのライブ窓へ時系列に追記し、処理中の動画時刻も表示します。各結果には動画開始からの経過時間、視覚抽出時間、各経路の処理時間、画面に結果が出るまでの遅延を表示します。結果JSONには画像自体を含めません。
