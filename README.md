...
同じ架空レース状態を入力し、選択した2モデルを並列実行して次の2経路を比較する再現可能な実験環境です。

- 通常LLMによる直接実況
- TypeSafe Jevによる構造化判断 → 同じLLMによる実況

OpenAI・Anthropic・Googleの対応モデルから2つを画面で選択できます。各fixtureにつきJev判断は1回だけ実行し、同じ判断を両モデルへ渡すため、モデル間の比較条件を揃えられます。固定fixture、モデルID、各処理のレイテンシ、Jevの構造化出力を結果JSONに保存できます。実在の映像・実況文・馬名は使いません。
...
- `MODEL_A` / `MODEL_B`: 初期選択する生成モデル。画面上でも変更可能
- `TRANSCRIPTION_MODEL`: Vercel AI Gatewayで利用する文字起こしモデル

キーが揃っていない場合は固定値を返すデモモードになります。実測には両方のキーが必要です。OpenAI・Anthropic・GoogleそれぞれのAPIキーは不要で、1つのAI Gatewayキーから対応モデルを切り替えます。
...
比較条件を変える場合は `src/lib/race.ts` のfixture、`.env.local` の `MODEL_A` / `MODEL_B`、`src/app/api/commentary/route.ts` のプロンプトを変更します。比較時はコミットSHAも一緒に記録してください。
